...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # lf = one_cycle(1, hyp['lrf'], epochs)  # cosine 1->hyp['lrf'] # 20210119 확인 기준 위 코드에서 수정됨
...
if __name__ == '__main__':
    # 훈련 전에 확인할 것들
    '''
    1. ./train.py 파일에서 훈련데이터가 잘 만들어졌는지 확인
     - ./yolov5/data/yolo_train, ./yolov5/data/yolo_val 폴더 내에 있음

    2. data.yaml 파일에서 경로, 클래스 수, 클래스 명이 잘 들어갔는지 확인
    ...
    # 여기에 각 파라미터 정의하고 실행
    model_size = 'yolov5s'
    exp_num = '20210120_0' # 실험 이름

    model_weights = './yolov5/weights/'+model_size+'.pt' # 사전훈련의 베이스로 사용한 모델 : Pre-Trained 모델 파일 경로 (pt 형식 파일)
    epochs = 1000 # 에폭 수 : 학습 몇 회 해볼건지
    ...
    cfg = './yolov5/models/'+model_size+'_custom.yaml' #yolov5 아키텍처 파일 경로 
    ...
    anchors_size = kmean_anchors(path='./yolov5/data/data.yaml', n=9, img_size=img_size, thr=4.0, gen=1000, verbose=True)
    print(anchors_size)
    print()
    ...
    # Evolve hyperparameters (optional)
        ...
        print()
        print('훈련은 다 되었으니 track.py으로 가서 실행하거라')
